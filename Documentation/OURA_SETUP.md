...
Request only the scopes used by the submitted version of VitalSync. Do not request email. The current import requests `heartrate` for heart rate, `daily` for sleep HRV and readiness temperature deviation, and `spo2` for sleep oxygen average. Existing connections need to choose Update Oura permissions in the app before importing the new categories.
